...
fn infer_forward_reference() {
    insta::assert_snapshot!(
        ...
        @"
    eq: forall g. fn(g, g) -> Bool
    lt: forall h. fn(h, h) -> Bool
    combine: forall g. fn(g, g) -> g
    sub: forall g. fn(g, g) -> g
    mul: forall g. fn(g, g) -> g
    div: forall g. fn(g, g) -> g
    neg: forall g. fn(g) -> g
    show: forall g. fn(g) -> String
    hash: forall g. fn(g) -> Int
    close: forall g. fn(~g) -> Unit
    peek: forall h. fn(h) -> h
...
fn infer_sum_constructor() {
    ...
    EQ: Ordering
    GT: Ordering
    Some: forall n. fn(n) -> Option[n]
    None: forall n. Option[n]
    wrap: forall z. fn(z) -> Option[z]
    "
    );
}
...
fn infer_scc_generalization_order() {
    ...
    GT: Ordering
    id: forall y. fn(y) -> y
    f: forall e1. fn(e1) -> e1
    g: forall j1. fn(j1) -> j1
    "
    );
}
...
fn infer_match_option() {
    ...
    Some: forall n. fn(n) -> Option[n]
    None: forall n. Option[n]
    unwrap_or: forall a1. fn(Option[a1], a1) -> a1
    "
    );
}
...
fn infer_match_nested_constructor() {
    ...
    EQ: Ordering
    GT: Ordering
    Cons: forall n. fn(n, List[n]) -> List[n]
    Nil: forall n. List[n]
...
fn infer_tuple_in_match() {
    ...
    EQ: Ordering
    GT: Ordering
    first: forall b1 c1. fn((b1, c1)) -> b1
    "
    );
}
...
fn infer_tuple_polymorphic() {
    ...
    EQ: Ordering
    GT: Ordering
    swap: forall b1 c1. fn((b1, c1)) -> (c1, b1)
    "
    );
}
...
fn infer_match_wrong_constructor() {
    insta::assert_snapshot!(
        ...
        @"TypeError: type mismatch: expected Color, found Option[c1]"
    );
}
...
fn explicit_hkt_type_param_generalized() {
    ...
    "#;
    insta::assert_snapshot!(infer_module_fn(src, "apply"), @"forall c1 d1. fn(c1[d1]) -> c1[d1]");
}
