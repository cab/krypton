...
fn impl_where_clause_constraints_are_stored_on_instance_defs() {
    ...
    let instance = modules[0]
        ...
        .iter()
        .find(|inst| {
            inst.trait_name.local_name == "MyEq" && inst.target_type_name == "Option$$Var0"
        })
...
fn infer_module_basic() {
    insta::assert_snapshot!(
        ...
        @"TypeError: function `add` requires trait `Semigroup` but has no declared type parameters"
    );
}
...
fn infer_module_returns_all_modules() {
    ...
    assert!(
        modules.iter().any(|m| m.module_path == "mylib"),
...
fn infer_module_stdlib_own_typed_module() {
    ...
    assert!(
        modules.iter().any(|m| m.module_path == "core/option"),
        "should have a TypedModule for core/option"
    );
}
...
fn shadowed_prelude_fn_note_on_ufcs_mismatch() {
    ...
    let note = te
        .note
        .as_ref()
        .expect("should have a note about shadowing");
    assert!(
