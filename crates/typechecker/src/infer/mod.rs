...
/// Walk a typed expression tree and collect pattern-bound owned variables.
/// For match expressions: keyed by match span, value is per-arm Vec of owned var names.
/// For let-pattern expressions: keyed by let-pattern span, value is single-element vec of vecs.
fn collect_match_arm_owned_vars(
    expr: &TypedExpr,
    info: &mut HashMap<Span, Vec<Vec<String>>>,
) {
    fn collect_owned_vars_from_pattern(pat: &TypedPattern, out: &mut Vec<String>) {
        match pat {
            ...
            TypedPattern::Constructor { args, .. } => {
                for arg in args {
                    collect_owned_vars_from_pattern(arg, out);
                }
            }
            TypedPattern::Tuple { elements, .. } => {
                for elem in elements {
                    collect_owned_vars_from_pattern(elem, out);
                }
            }
            TypedPattern::StructPat { fields, .. } => {
                for (_, pat) in fields {
                    collect_owned_vars_from_pattern(pat, out);
                }
            }
        ...
        match &expr.kind {
            TypedExprKind::Match { scrutinee, arms } => {
                ...
                for arm in arms {
                    ...
                    collect_owned_vars_from_pattern(&arm.pattern, &mut owned_vars);
                    per_arm.push(owned_vars);
                    work.push(&arm.body);
                }
                if per_arm.iter().any(|v| !v.is_empty()) {
                    info.insert(expr.span, per_arm);
                }
            }
            TypedExprKind::LetPattern { pattern, value, body } => {
                let mut owned_vars = Vec::new();
                collect_owned_vars_from_pattern(pattern, &mut owned_vars);
                if !owned_vars.is_empty() {
                    info.insert(expr.span, vec![owned_vars]);
                }
                work.push(value);
                if let Some(b) = body {
                    work.push(b);
            ...
            TypedExprKind::TypeApp { expr, .. } => work.push(expr),
            TypedExprKind::If { cond, then_, else_ } => {
                work.push(cond); work.push(then_); work.push(else_);
            }
            TypedExprKind::Let { value, body, .. } => {
                work.push(value);
...
/// Walk a typed expression tree and collect spans of field access expressions
/// whose result type is `Type::Own(...)`.
fn collect_field_access_owned(
    expr: &TypedExpr,
    info: &mut HashSet<Span>,
) {
    let mut work: Vec<&TypedExpr> = Vec::with_capacity(16);
    work.push(expr);
    while let Some(expr) = work.pop() {
...
impl ModuleInferenceState {
    ...
    fn assemble_typed_module(
        ...
        for func in &functions {
            ...
            collect_match_arm_owned_vars(&func.body, &mut match_arm_owned_vars);
            collect_field_access_owned(&func.body, &mut field_access_owned);
        }
