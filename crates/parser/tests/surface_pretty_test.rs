...
use krypton_parser::surface_pretty::{surface_pretty_print, surface_pretty_print_with, SurfacePrettyConfig};
...
fn assert_surface_roundtrip(input: &str) {
    let (module1, errors1) = surface_parse(input);
    assert!(
        errors1.is_empty(),
        "parse errors on input: {errors1:?}\ninput: {input}"
    );

    let printed = surface_pretty_print(&module1);
    let (module2, errors2) = surface_parse(&printed);
    assert!(
        errors2.is_empty(),
...
fn roundtrip_precedence_add_mul() {
    ...
    let (module1, _) = surface_parse(input);
    let printed = surface_pretty_print(&module1);
    assert!(
...
fun main() -> Int {
    ...
    let (module, errors) = surface_parse(input);
    assert!(errors.is_empty(), "parse errors: {errors:?}");
    let output = surface_pretty_print(&module);
...
fn configurable_indent_width() {
    ...
    let config = SurfacePrettyConfig { indent_width: 2 };
    let output = surface_pretty_print_with(&module, &config);
