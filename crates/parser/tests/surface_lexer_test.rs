...
use krypton_parser::surface_lexer::surface_lexer;

fn surface_lex(input: &str) -> Vec<Token<'_>> {
    let (tokens, errors) = surface_lexer().parse(input).into_output_errors();
...
#[test]
fn test_spans() {
    let (tokens, _) = surface_lexer().parse("fun foo").into_output_errors();
...
fn test_multi_char_operator_spans() {
    let (tokens, _) = surface_lexer().parse("== =>").into_output_errors();
    let tokens = tokens.unwrap();
    // '==' at 0..2
    assert_eq!(tokens[0].1.start, 0);
