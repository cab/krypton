...
#[test]
fn test_multi_param_trait_decl() {
    let src = r#"
trait Convert[a, b] {
    fun convert(x: a) -> b
}
"#;
    ...
    assert!(errors.is_empty(), "errors: {errors:?}");
    match &module.decls[0] {
        krypton_parser::ast::Decl::DefTrait {
            name, type_params, ..
        } => {
...
#[test]
fn test_multi_param_impl_decl() {
    let src = r#"
impl Convert[Int, String] {
    fun convert(x: Int) -> String = show(x)
}
"#;
    let (module, errors) = parse(src);
    assert!(errors.is_empty(), "errors: {errors:?}");
    match &module.decls[0] {
        krypton_parser::ast::Decl::DefImpl {
            trait_name,
            type_args,
            ..
        } => {
...
#[test]
fn test_multi_param_impl_with_where() {
    let src = r#"
impl[a] Convert[Array[a], String] where a: Show {
    fun convert(x: Array[a]) -> String = show(x)
}
"#;
    let (module, errors) = parse(src);
    assert!(errors.is_empty(), "errors: {errors:?}");
