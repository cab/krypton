...
impl<'a> LowerCtx<'a> {
    ...
    #[inline]
    fn lower_discharge(&mut self, expr: &TypedExpr, inner: &TypedExpr) -> Result<Expr, LowerError> {
    ...
    #[inline]
    fn lower_lit(&mut self, expr: &TypedExpr, lit: &Lit) -> Result<Expr, LowerError> {
        Ok(atom_expr_at(
            expr.span,
            expr.ty.clone().into(),
    ...
    #[inline]
    fn lower_var(&mut self, expr: &TypedExpr, name: &str) -> Result<Expr, LowerError> {
        if let Some((_binding_name, constructor_ref)) = resolved_constructor_ref(expr) {
            match constructor_ref.kind {
                typed_ast::ConstructorKind::Variant => {
                    let variant_ref =
    ...
    #[inline]
    fn lower_tuple(&mut self, expr: &TypedExpr, elems: &[TypedExpr]) -> Result<Expr, LowerError> {
        let result_ty = expr.ty.clone();
        let span = expr.span;
    ...
    #[inline]
    fn lower_recur(&mut self, expr: &TypedExpr, args: &[TypedExpr]) -> Result<Expr, LowerError> {
    ...
    #[inline]
    fn lower_vec_lit(&mut self, expr: &TypedExpr, elems: &[TypedExpr]) -> Result<Expr, LowerError> {
        let result_ty = expr.ty.clone();
        let element_type = extract_vec_element_type(&expr.ty)?;
        let span = expr.span;
        self.lower_atoms_then(elems, vec![], |ctx, atoms| {
