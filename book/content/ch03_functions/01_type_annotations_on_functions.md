In the previous lesson, we explored Defining Functions. Now we are going to take a look at Type Annotations On Functions, which is an important concept in Krypton. As you may already be aware, many programming languages have some way of talking about writing explicit parameter and return types on function definitions, and Krypton is no exception to that general pattern. It is worth mentioning, however, that the exact spelling still matters, because small syntax choices tend to shape how comfortably a reader can move through later code.

The example below demonstrates that parameter types go inside the parameter list and the return type follows `->`. Note that this is sometimes more verbose than inference, but it is also the most direct way to document a public function contract in code. This may sound somewhat ordinary, and in a sense it is, but that is also why it is useful to see it written out plainly before the lessons get more layered. Interestingly, this is somewhat similar to annotated signatures in many statically typed languages, although the Krypton version has its own punctuation and naming choices that should be read carefully.

With that in mind, let us now turn our attention to what the program is doing at runtime and at compile time. The point is not only that the example works, but also that the surface shape of the code communicates the rule being introduced here in a fairly direct way. It is generally considered best practice to make one small change and see what changes elsewhere, even if the change is something as minor as a different literal, branch, field name, or type annotation. Feel free to experiment with the code above and observe what happens.


```krypton
fun greet(name: String) -> String = "Hello, " + name
fun twice(n: Int) -> Int = n * 2

fun main() {
  println(greet("Ada"))
  println(twice(21))
}

```
