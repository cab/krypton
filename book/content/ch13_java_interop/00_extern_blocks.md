In the previous lesson, we explored Koan Modules. Now we are going to take a look at Extern Blocks, which is an important concept in Krypton. As you may already be aware, many programming languages have some way of talking about declaring foreign functions so Krypton code can call into Java APIs, and Krypton is no exception to that general pattern. It is worth mentioning, however, that the exact spelling still matters, because small syntax choices tend to shape how comfortably a reader can move through later code.

The example below demonstrates that an `extern java "..."` block names the host class and then lists the functions Krypton can call from it. Note that the example stays with very familiar math functions because the interop syntax itself is already enough new information for one lesson. This may sound somewhat ordinary, and in a sense it is, but that is also why it is useful to see it written out plainly before the lessons get more layered. Interestingly, this is somewhat similar to foreign function declarations in statically typed languages, although the Krypton version has its own punctuation and naming choices that should be read carefully.

With that in mind, let us now turn our attention to what the program is doing at runtime and at compile time. The point is not only that the example works, but also that the surface shape of the code communicates the rule being introduced here in a fairly direct way. It is generally considered best practice to make one small change and see what changes elsewhere, even if the change is something as minor as a different literal, branch, field name, or type annotation. Feel free to experiment with the code above and observe what happens.


```krypton
extern java "java.lang.Math" {
  fun abs(n: Int) -> Int
  fun max(a: Int, b: Int) -> Int
}

fun main() {
  println(abs(-4))
  println(max(3, 9))
}

```
